...
/** Usage:
    ...
    std::string key = "key";
    std::string md5hmac  = hmac< HL_MD5  >(msg, key);
    std::string sha1hmac = hmac< HL_SHA1 >(msg, key);
    std::string sha2hmac = hmac<HL_SHA256>(msg, key);

    Note:
    To keep my code simple, HMAC computation currently needs the whole message at once.
    This is in contrast to the hashes HL_MD5, HL_SHA1, etc. where an add() method is available
    for incremental computation.
    You can use any hash for HMAC as long as it provides:
    - constant HashMethod::BlockSize (typically 64)
    - constant HashMethod::HashBytes (length of hash in bytes, e.g. 20 for HL_SHA1)
...
/// compute HMAC hash of data and key using HL_MD5, HL_SHA1 or HL_SHA256
