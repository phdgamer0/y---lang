...
/// process 64 bytes
void HL_SHA256::processBlock(const void* data)
{
  // get last hash
  uint32_t a = m_hash[0];
  uint32_t b = m_hash[1];
...
void HL_SHA256::add(const void* data, size_t numBytes)
{
...
void HL_SHA256::getHash(unsigned char buffer[HL_SHA256::HashBytes])
{
...
/// compute HL_SHA256 of a memory block
std::string HL_SHA256::operator()(const void* data, size_t numBytes)
{
  reset();
  add(data, numBytes);
  return getHash();
...
/// compute HL_SHA256 of a string, excluding final zero
std::string HL_SHA256::operator()(const std::string& text)
{
  reset();
