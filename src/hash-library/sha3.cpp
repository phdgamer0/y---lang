...
void HL_SHA3::processBlock(const void* data)
{
...
void HL_SHA3::add(const void* data, size_t numBytes)
{
...
std::string HL_SHA3::getHash()
{
  ...
  // HL_SHA3-224's last entry in m_hash provides only 32 bits instead of 64 bits
...
/// compute HL_SHA3 of a memory block
std::string HL_SHA3::operator()(const void* data, size_t numBytes)
{
  reset();
  add(data, numBytes);
  return getHash();
...
/// compute HL_SHA3 of a string, excluding final zero
std::string HL_SHA3::operator()(const std::string& text)
{
  reset();
