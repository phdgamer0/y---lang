...
void HL_Keccak::processBlock(const void* data)
{
...
void HL_Keccak::add(const void* data, size_t numBytes)
{
...
/// compute HL_Keccak hash of a memory block
std::string HL_Keccak::operator()(const void* data, size_t numBytes)
{
  reset();
  add(data, numBytes);
  return getHash();
...
/// compute HL_Keccak hash of a string, excluding final zero
std::string HL_Keccak::operator()(const std::string& text)
{
  reset();
