...
/// process 64 bytes
void HL_MD5::processBlock(const void* data)
{
  // get last hash
  uint32_t a = m_hash[0];
  uint32_t b = m_hash[1];
...
void HL_MD5::add(const void* data, size_t numBytes)
{
...
void HL_MD5::getHash(unsigned char buffer[HL_MD5::HashBytes])
{
...
/// compute HL_MD5 of a memory block
std::string HL_MD5::operator()(const void* data, size_t numBytes)
{
  reset();
  add(data, numBytes);
  return getHash();
...
/// compute HL_MD5 of a string, excluding final zero
std::string HL_MD5::operator()(const std::string& text)
{
  reset();
